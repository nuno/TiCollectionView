 ...
 * See {@link TiGridView#addHeaderView(View, Object, boolean)}
 * See {@link TiGridView#addFooterView(View, Object, boolean)}
 */
public class TiGridView extends GridView {
    ...
    private static final String TAG = "TiGridView";
    ...
    public TiGridView(Context context) {
        super(context);
        initHeaderGridView();
    }

    public TiGridView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initHeaderGridView();
    }

    public TiGridView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (adapter != null && adapter instanceof TiGridAdapter) {
            ((TiGridAdapter) adapter).setNumColumns(getNumColumnsCompatible());
            ((TiGridAdapter) adapter).setRowHeight(getRowHeight());
        }
    }
    ...
    public void addHeaderView(View v, Object data, boolean isSelectable) {
        ...
        if (adapter != null) {
            ((TiGridAdapter) adapter).notifyDataSetChanged();
        }
    }
    ...
    public void addFooterView(View v, Object data, boolean isSelectable) {
        ...
        if (mAdapter != null && !(mAdapter instanceof TiGridAdapter)) {
            throw new IllegalStateException(
                    "Cannot add header view to grid -- setAdapter has already been called.");
        }
        ...
        if (mAdapter != null) {
            ((TiGridAdapter) mAdapter).notifyDataSetChanged();
        }
    }
    ...
    public boolean removeHeaderView(View v) {
        if (mHeaderViewInfos.size() > 0) {
            ...
            ListAdapter adapter = getAdapter();
            if (adapter != null && ((TiGridAdapter) adapter).removeHeader(v)) {
                result = true;
            }
    ...
    public boolean removeFooterView(View v) {
        if (mFooterViewInfos.size() > 0) {
            boolean result = false;
            ListAdapter adapter = getAdapter();
            if (adapter != null && ((TiGridAdapter) adapter).removeFooter(v)) {
                result = true;
            }
    ...
    public void setAdapter(ListAdapter adapter) {
        if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
        	Log.i(TAG, "setAdapter called");
            TiGridAdapter tiGridAdapter = new TiGridAdapter(mHeaderViewInfos, mFooterViewInfos, adapter);
            int numColumns = getNumColumnsCompatible();
            if (numColumns > 1) {
            	tiGridAdapter.setNumColumns(numColumns);
            }
            tiGridAdapter.setRowHeight(getRowHeight());
            super.setAdapter(tiGridAdapter);
        } else {
    ...
    private class FullWidthFixedViewLayout extends FrameLayout {
        ...
        protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
            int realLeft = TiGridView.this.getPaddingLeft() + getPaddingLeft();
        ...
        @Override
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            int targetWidth = TiGridView.this.getMeasuredWidth()
                    - TiGridView.this.getPaddingLeft()
                    - TiGridView.this.getPaddingRight();
    ...
    public void setNumColumns(int numColumns) {
        ...
        ListAdapter adapter = getAdapter();
        if (adapter != null && adapter instanceof TiGridAdapter) {
            ((TiGridAdapter) adapter).setNumColumns(numColumns);
        }
    }
     ...
     */
    private static class TiGridAdapter implements WrapperListAdapter, Filterable {
        ...
        public TiGridAdapter(ArrayList<FixedViewInfo> headerViewInfos, ArrayList<FixedViewInfo> footViewInfos, ListAdapter adapter) {
            mAdapter = adapter;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            //if (DEBUG) {
            Log.d(TAG, String.format("getView: %s, reused: %s", position, convertView == null));
            //}
            // Header (negative positions will throw an ArrayIndexOutOfBoundsException)
            int numHeadersAndPlaceholders = getHeadersCount() * mNumColumns;
            if (position < numHeadersAndPlaceholders) {
        ...
        public int getItemViewType(int position) {
            ...
            if (DEBUG) {
                Log.d(TAG, String.format("getItemViewType: pos: %s, result: %s", position, type, mCachePlaceHoldView, mCacheFirstHeaderView));
            }
            return type;
        }
        ...
        public int getViewTypeCount() {
            ...
            if (DEBUG) {
                Log.d(TAG, String.format("getViewTypeCount: %s", count));
            }
            return count;
        }
